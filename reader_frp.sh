...
frp_get_latest() {
	...
	([[ $? != 0 ]] || [[ -z "$tag2" ]]) && (echo "tag2 get failed!"; exit 255)

	[[ "${file_name}" =~ "${tag2}" ]] && return
	...
	[[ $? != 0 ]] && (rm -f $new_tar; exit 222)

	[[ "${file_name}" != "frp*${FRP_ARCH}" ]] &&  rm -rf ${file_name}*

	tar -xf $new_tar || (rm -rf frp*${FRP_ARCH}; exit 255)
}
...
frp_get_static() {
	...
	tar -xf $static_ver || (rm -rf frp*${FRP_ARCH}; exit 255)
}
...
show_help() {
  ...
  echo '    -u                        Frp app automatic update.'
  exit 0
